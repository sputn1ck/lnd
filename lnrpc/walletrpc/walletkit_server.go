...
func (w *WalletKit) SubmitPackage(ctx context.Context,
	...
	// Validate that there's at least one parent transaction. The underlying
	// PackageBroadcaster might also do this, but good to check early.
	...
	// Attempt to submit the package.
	submittedTxIDs, err := w.cfg.Wallet.SubmitPackage(
		parents, childTx,
	...
	respTxIDs := make([]string, 0, len(submittedTxIDs))
	for _, txid := range submittedTxIDs {
		if txid != nil {
			respTxIDs = append(respTxIDs, txid.String())
		}
	}

	return &SubmitPackageResponse{PackageTxids: respTxIDs}, nil
}
